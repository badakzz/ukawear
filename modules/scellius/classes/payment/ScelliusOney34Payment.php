...
class ScelliusOney34Payment extends AbstractScelliusPayment
{
    const ONEY34_THREE_TIMES_MAX_FEES = 10;
    const ONEY34_FOUR_TIMES_MAX_FEES = 20;

    protected $prefix = 'SCELLIUS_ONEY34_';
    ...
    public function validate($cart, $data = array())
    {
        ...
        $errors = ScelliusTools::checkAddress($billing_address, 'billing', 'oney');

        if (empty($errors)) {
            // Billing address is valid, check delivery address.
            $delivery_address = new Address((int) $cart->id_address_delivery);

            $errors = ScelliusTools::checkAddress($delivery_address, 'delivery', 'oney');
        }
    ...
    public static function getAvailableOptions($cart)
    {
        ...
        $options = @unserialize(Configuration::get('SCELLIUS_ONEY34_OPTIONS'));
    ...
    public function getTplVars($cart)
    {
        ...
        $vars['scellius_oney_options'] = self::getAvailableOptions($cart);
