...
class ScelliusMultiPayment extends AbstractScelliusPayment
{
    protected $prefix = 'SCELLIUS_MULTI_';
    ...
    public static function getAvailableOptions($cart = null)
    {
        ...
        $options = @unserialize(Configuration::get('SCELLIUS_MULTI_OPTIONS'));
    ...
    public function getTplVars($cart)
    {
        ...
        $vars['scellius_multi_options'] = self::getAvailableOptions($cart);

        $entry_mode = Configuration::get($this->prefix . 'CARD_MODE');
        $vars['scellius_multi_card_mode'] = $entry_mode;

        if ($entry_mode === '2') {
            $vars['scellius_avail_cards'] = $this->getPaymentCards();
        }
    ...
    {
        $all_cards = ScelliusTools::getSupportedMultiCardTypes();
