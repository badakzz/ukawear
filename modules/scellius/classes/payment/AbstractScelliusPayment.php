...
abstract class AbstractScelliusPayment
{
    const SCELLIUS_CART_MAX_NB_PRODUCTS = 85;
    ...
    {
        return array(
            'scellius_title' => $this->getTitle((int) $cart->id_lang),
            'scellius_logo' => _MODULE_DIR_ . 'scellius/views/img/' . $this->getLogo()
        );
    }
    ...
    public function getPaymentOption($cart)
    {
        ...
        $option->setCallToActionText($this->getTitle((int) $cart->id_lang))
                ...
                ->setLogo(_MODULE_DIR_ . 'scellius/views/img/' . $this->getLogo());

        if (! $this->hasForm()) {
            $option->setAction(Context::getContext()->link->getModuleLink('scellius', 'redirect', array(), true));

            $inputs = array(
                array('type' => 'hidden', 'name' => 'scellius_payment_type', 'value' => $this->name)
            );
    ...
    public function prepareRequest($cart, $data = array())
    {
        ...
        $colissimo_address = ScelliusTools::getColissimoDeliveryAddress($cart); // Get SoColissimo delivery address.
        ...
        ScelliusTools::getLogger()->logInfo("Form data generation for cart #{$cart->id} with {$this->name} submodule.");

        require_once _PS_MODULE_DIR_ . 'scellius/classes/ScelliusRequest.php';
        /* @var $request ScelliusRequest */
        $request = new ScelliusRequest();

        $contrib = ScelliusTools::getDefault('CMS_IDENTIFIER') . '_' . ScelliusTools::getDefault('PLUGIN_VERSION') . '/' . _PS_VERSION_ . '/' . PHP_VERSION;
        ...
        $cart_currency = new Currency((int) $cart->id_currency);
        $currency = ScelliusApi::findCurrencyByAlphaCode($cart_currency->iso_code);
        ...
        if (Configuration::get('SCELLIUS_COMMON_CATEGORY') !== 'CUSTOM_MAPPING') {
            $category = Configuration::get('SCELLIUS_COMMON_CATEGORY');
        } else {
            $oney_categories = @unserialize(Configuration::get('SCELLIUS_CATEGORY_MAPPING'));
        }

        $subtotal = 0;
        $products = $cart->getProducts(true);
        if (count($products) <= self::SCELLIUS_CART_MAX_NB_PRODUCTS || $this->proposeOney($data)) {
            ...
            foreach ($products as $product) {
                ...
                if ((! $this->force_local_cart_data && (Configuration::get('SCELLIUS_SEND_CART_DETAIL') === 'True')) || $this->needs_cart_data) {
                    $request->addProduct(
        ...
        if (Configuration::get('SCELLIUS_SEND_SHIP_DATA') === 'True' || $this->proposeOney($data)) {
        ...
        // Activate 3DS?
        $threeds_mpi = null;
        $threeds_min_amount_options = @unserialize(Configuration::get('SCELLIUS_3DS_MIN_AMOUNT'));
        ...
        // Return URL.
        $request->set('url_return', Context::getContext()->link->getModuleLink('scellius', 'submit', array(), true));

        return $request;
    }

    private function setAdditionalData($cart, $delivery_address, &$scellius_request, $use_oney = false, $isOney34 = false)
    {
        // Oney delivery options defined in admin panel.
        $shipping_options = @unserialize(Configuration::get('SCELLIUS_ONEY_SHIP_OPTIONS'));
        ...
        if ($cart->isVirtualCart() || ! isset($carrier_id) || ! is_array($shipping_options) || empty($shipping_options)) {
            ...
            $scellius_request->set('ship_to_type', 'ETICKET');
            $scellius_request->set('ship_to_speed', 'EXPRESS');

            $scellius_request->set(
            ...
            $scellius_request->set('ship_to_type', 'RELAY_POINT');
            $scellius_request->set('ship_to_speed', 'STANDARD');
            ...
            switch (true) {
                ...
                case (self::isColissimoRelay($carrier_id) && $delivery_address->company /* Relay point. */):
                    ...
                    // Already set address.
                    $city = $scellius_request->get('ship_to_city');
                    $zipcode = $scellius_request->get('ship_to_zip');
                    $country = $scellius_request->get('ship_to_country');
                    break;
            ...
            $scellius_request->set('ship_to_street', preg_replace($address_not_allowed_chars, ' ', $address));
            $scellius_request->set('ship_to_street2', null);
            $scellius_request->set('ship_to_zip', $zipcode);
            $scellius_request->set('ship_to_city', preg_replace($not_allowed_chars, ' ', $city));
            $scellius_request->set('ship_to_state', null);
            $scellius_request->set('ship_to_country', $country);

            $delivery_company = preg_replace($not_allowed_chars, ' ', $address . ' ' . $zipcode . ' ' . $city);
            $scellius_request->set('ship_to_delivery_company_name', $delivery_company);
        } else {
            // Other cases
            ...
            $scellius_request->set('ship_to_type', $delivery_type);
            $scellius_request->set('ship_to_speed', $delivery_speed);
            ...
            if ($delivery_type === 'RECLAIM_IN_SHOP') {
                ...
                $scellius_request->set('ship_to_street', $shop_name . ' ' . $shipping_options[$carrier_id]['address']);
                $scellius_request->set('ship_to_street2', null);
                $scellius_request->set('ship_to_zip', $shipping_options[$carrier_id]['zip']);
                $scellius_request->set('ship_to_city', $shipping_options[$carrier_id]['city']);
                $scellius_request->set('ship_to_country', 'FR');
            ...
            if ($shipping_options[$carrier_id]['speed'] === 'PRIORITY') {
                $scellius_request->set('ship_to_delay', $shipping_options[$carrier_id]['delay']);
            }

            $scellius_request->set('ship_to_delivery_company_name', $company_name);
        }

        if ($use_oney) {
            // Modify address to send it to Oney.

            if ($scellius_request->get('ship_to_street')) { // If there is a delivery address.
                $scellius_request->set('ship_to_status', 'PRIVATE'); // By default PrestaShop doesn't manage customer type.

                $address = $scellius_request->get('ship_to_street') . ' ' . $scellius_request->get('ship_to_street2');

                $scellius_request->set('ship_to_street', preg_replace($address_not_allowed_chars, ' ', $address));
                $scellius_request->set('ship_to_street2', $relay_point_name);
            ...
            $scellius_request->set('cust_status', 'PRIVATE');

            // Send FR even address is in DOM-TOM unless form is rejected.
            $scellius_request->set('cust_country', 'FR');
        }
    }
    ...
    protected function l($string)
    {
        ...
        return $scellius->l($string, Tools::strtolower(get_class($this)));
    }
}
