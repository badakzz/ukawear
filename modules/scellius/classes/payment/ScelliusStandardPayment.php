...
class ScelliusStandardPayment extends AbstractScelliusPayment
{
    protected $prefix = 'SCELLIUS_STD_';
    ...
    public function isAvailable($cart)
    {
        ...
        if ($this->proposeOney()) {
            return ScelliusTools::checkOneyRequirements($cart, 'FacilyPay Oney');
        }
    ...
    public function getTplVars($cart)
    {
        ...
        $vars['scellius_std_card_data_mode'] = $entry_mode;
        ...
        $vars['scellius_saved_identifier'] = false;
        $vars['scellius_saved_payment_mean'] = '';

        $vars['scellius_rest_form_token'] = '';
        $vars['scellius_rest_identifier_token'] = '';
        $vars['scellius_rest_popin'] = false;

        if ((Configuration::get('SCELLIUS_STD_1_CLICK_PAYMENT') === 'True') && $saved_identifier) {
            $vars['scellius_saved_identifier'] = true;
            $vars['scellius_saved_payment_mean'] = isset($customers_config[$cart->id_customer][$this->name]['m']) ?
        ...
        if ($entry_mode === '2' /* Card type on website. */) {
            ...
            $vars['scellius_can_cancel_iframe'] = (Configuration::get($this->prefix . 'CANCEL_IFRAME') === 'True');
            $this->tpl_name = 'payment_std_iframe.tpl';
        } elseif ($entry_mode === '5' /* REST API. */) {
            $vars['scellius_rest_popin'] = Configuration::get('SCELLIUS_STD_REST_DISPLAY_MODE') === 'popin';

            $form_token = $this->getFormToken($cart);

            if ($form_token) {
                // REST API params.
                $vars['scellius_rest_form_token'] = $form_token;
                $vars['scellius_rest_identifier_token'] = $form_token;
                ...
                if (Configuration::get('SCELLIUS_STD_1_CLICK_PAYMENT') === 'True' && $saved_identifier) {
                    ...
                    if ($identifier_token) {
                        $vars['scellius_rest_identifier_token'] = $identifier_token;
                    }
                }
                ...
                $vars['scellius_std_card_data_mode'] = '1';
            }
        }
    ...
    private function getPaymentCards()
    {
        ...
        if (! empty($cards)) {
            ...
            $cards = array_keys(ScelliusTools::getSupportedCardTypes());
        }

        if ($this->proposeOney()) {
            $cards[] = (Configuration::get('SCELLIUS_MODE') === 'TEST') ? 'ONEY_SANDBOX' : 'ONEY';
        }
    ...
    public function prepareRequest($cart, $data = array())
    {
        ...
        if (Configuration::get('SCELLIUS_STD_1_CLICK_PAYMENT') === 'True') {
            ...
            if ($saved_identifier) {
                ...
                ScelliusTools::getLogger()->logInfo('Customer ' . $request->get('cust_email') . ' will be asked for card data registration on payment page.');
    ...
    public function getFormToken($cart, $use_identifier = false)
    {
        ...
        $currency = ScelliusApi::findCurrencyByNumCode($this->getEscapedVar($request, 'currency'));
        ...
        if ($use_identifier) {
            ScelliusTools::getLogger()->logInfo("Customer {$cust_mail} has an identifier. Use it for payment of cart #{$cart_id}.");

            $customers_config = @unserialize(Configuration::get('SCELLIUS_CUSTOMERS_CONFIG'));
            $saved_identifier = isset($customers_config[$cart->id_customer][$this->name]['n']) ? $customers_config[$cart->id_customer][$this->name]['n'] : '';
            $params['paymentMethodToken'] = $saved_identifier;
        } elseif (Configuration::get('SCELLIUS_STD_1_CLICK_PAYMENT') === 'True' && $this->getEscapedVar($request, 'cust_id')) {
            // 1-Click enabled and customer logged-in, let's ask customer for card data registration.
            ScelliusTools::getLogger()->logInfo("Customer {$cust_mail} will be asked for card data registration on payment page for order #{$cart_id}.");
            $params['formAction'] = 'ASK_REGISTER_PAY';
        }

        $test_mode = Configuration::get('SCELLIUS_MODE') === 'TEST';
        $key = $test_mode ? Configuration::get('SCELLIUS_PRIVKEY_TEST') : Configuration::get('SCELLIUS_PRIVKEY_PROD');
        $site_id = Configuration::get('SCELLIUS_SITE_ID');

        require_once _PS_MODULE_DIR_ . 'scellius/classes/ScelliusRest.php';

        $return = false;
        try {
            $client = new ScelliusRest(ScelliusTools::getDefault('REST_URL'), $site_id, $key);
            $result = $client->post('V4/Charge/CreatePayment', json_encode($params));

            if ($result['status'] !== 'SUCCESS') {
                ScelliusTools::getLogger()->logError("Error while creating payment form token for cart #$cart_id: " . $result['answer']['errorMessage']
                    . ' (' . $result['answer']['errorCode'] . ').');

                if (isset($result['answer']['detailedErrorMessage']) && ! empty($result['answer']['detailedErrorMessage'])) {
                    ScelliusTools::getLogger()->logError('Detailed message: ' . $result['answer']['detailedErrorMessage']
                        . ' (' . $result['answer']['detailedErrorCode'] . ').');
                }
            } else {
                // Payment form token created successfully.
                ScelliusTools::getLogger()->logInfo("Form token created successfully for cart #$cart_id.");
                $return = $result['answer']['formToken'];
            }
        } catch (Exception $e) {
            ScelliusTools::getLogger()->logError($e->getMessage());
        }
