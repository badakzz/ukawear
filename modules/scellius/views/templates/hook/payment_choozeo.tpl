...
<form action="{$link->getModuleLink('scellius', 'redirect', array(), true)|escape:'html':'UTF-8'}" method="post" id="scellius_choozeo" style="margin-left: 2.875rem; margin-top: 1.25rem; margin-bottom: 1rem;">
  ...
  {foreach from=$scellius_choozeo_options key="key" item="option"}
    <label class="scellius_card_click" for="scellius_card_type_{$key|escape:'html':'UTF-8'}">
      <input type="radio" name="scellius_card_type" id="scellius_card_type_{$key|escape:'html':'UTF-8'}" value="{$key|escape:'html':'UTF-8'}" {if $first == true} checked="checked"{/if} />
...
<![if IE]>
  ...
  // <![CDATA[
    $('#scellius_choozeo label.scellius_card_click img').on('click', function(e) {
      $(this).parent().click();
    });
  // ]]>
  </script>
