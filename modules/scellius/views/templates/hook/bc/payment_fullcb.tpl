...
{if {$scellius_fullcb_options|@count} == 0}
  <div class="payment_module scellius {$scellius_tag|escape:'html':'UTF-8'}">
    <a href="javascript: $('#scellius_fullcb').submit();" title="{l s='Click here to pay with FullCB' mod='scellius'}">
      <img class="logo" src="{$scellius_logo|escape:'html':'UTF-8'}" alt="Scellius" />{$scellius_title|escape:'html':'UTF-8'}

      <form action="{$link->getModuleLink('scellius', 'redirect', array(), true)|escape:'html':'UTF-8'}" method="post" id="scellius_fullcb">
        <input type="hidden" name="scellius_payment_type" value="fullcb" />
      </form>
    </a>
  </div>
{else}
      ...
      <form action="{$link->getModuleLink('scellius', 'redirect', array(), true)|escape:'html':'UTF-8'}" method="post" id="scellius_fullcb">
        ...
        {foreach from=$scellius_fullcb_options key="key" item="option"}
          <div style="padding-bottom: 5px;">
            {if $scellius_fullcb_options|@count == 1}
              <input type="hidden" id="scellius_fullcb_option_{$key|escape:'html':'UTF-8'}" name="scellius_card_type" value="{$key|escape:'html':'UTF-8'}" >
            {else}
              <input type="radio"
                     id="scellius_fullcb_option_{$key|escape:'html':'UTF-8'}"
                     name="scellius_card_type"
                     value="{$key|escape:'html':'UTF-8'}"
                     style="vertical-align: middle;"
                     {if $first == true} checked="checked"{/if}
                     onclick="javascript: $('.scellius_fullcb_review').hide(); $('#scellius_fullcb_review_{$key|escape:'html':'UTF-8'}').show();">
            {/if}

            <label for="scellius_fullcb_option_{$key|escape:'html':'UTF-8'}" style="display: inline;">
              <span style="vertical-align: middle;">{$option.localized_label|escape:'html':'UTF-8'}</span>
            </label>

            <table class="scellius_fullcb_review scellius_review" id="scellius_fullcb_review_{$key|escape:'html':'UTF-8'}" {if $first != true} style="display: none;"{/if}>
              <tr>
                <td>
                  <table>
                    <tbody>
                      <tr>
                        <td>{l s='Order amount :' mod='scellius'}</td>
                        <td class="amount">{$option.order_amount|escape:'html':'UTF-8'}</td>
                      </tr>
                      <tr>
                        <td>{l s='Fees :' mod='scellius'}</td>
                      ...
                      <tr>
                        <td>{l s='Total amount :' mod='scellius'}</td>
                ...
                <td>
                  <table>
                    <tbody>
                      <tr>
                        <th colspan="2">{l s='Installments' mod='scellius'}</th>
                      </tr>
