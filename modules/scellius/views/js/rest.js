...
$(function() {
    setTimeout(function() {
        if ($('#cgv').length) {
            if ($('#cgv').is(':checked')) {
                $('.scellius .kr-payment-button').removeAttr('disabled');
            } else {
                // Unchecked CVG, disable payment button.
                $('.scellius .kr-payment-button').attr('disabled', 'disabled');
            }
        }

        $('.scellius .kr-payment-button').click(function(e) {
            $('.scellius .kr-form-error').html('');
        });

        scelliusInitRestEvents();
    }, 0);
});

var SCELLIUS_DEFAULT_MESSAGES = ['CLIENT_300', 'CLIENT_304', 'CLIENT_502', 'PSP_539']; // Use default messages for these errors.
var SCELLIUS_RECOVERABLE_ERRORS = [
...
var scelliusInitRestEvents = function() {
    KR.onError(function(e) {
        $('.scellius .processing').css('display', 'none');
        $('#scellius_oneclick_payment_description').show();

        if ($('#scellius_standard').length && $('#scellius_standard').data('submitted')) {
        ...
        if (SCELLIUS_RECOVERABLE_ERRORS.indexOf(e.errorCode) === -1) {
            setTimeout(function() {
        ...
        var msg = '';
        if (SCELLIUS_DEFAULT_MESSAGES.indexOf(e.errorCode) > -1) {
            msg = e.errorMessage + (e.errorMessage.endsWith('.') ? '' : '.');
        } else {
            msg = scelliusTranslate(e.errorCode);
        }

        $('.scellius .kr-form-error').html('<span style="color: red;"><span>' + msg + '</span></span>');
    });
...
var scelliusTranslate = function(code) {
    var lang = SCELLIUS_LANGUAGE; // Global variable that contains current language.
    var messages = SCELLIUS_ERROR_MESSAGES.hasOwnProperty(lang) ? SCELLIUS_ERROR_MESSAGES[lang] : SCELLIUS_ERROR_MESSAGES['en'];
