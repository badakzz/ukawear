 ...
 */
class ScelliusSubmitModuleFrontController extends ModuleFrontController
{
    public $ssl = true;
    ...
    public function postProcess()
    {
        ...
        if ($this->iframe) {
            unset($this->context->cookie->scelliusCartId); // Used in iframe mode.
        }
    ...
    private function processPaymentReturn()
    {
        ...
        if (! $response->isAuthentified()) {
            ...
            $this->logger->logError("{$ip} tries to access module/scellius/submit page without valid signature with parameters: " . print_r($_REQUEST, true));
        ...
        if ($order_id == false) {
            ...
            if ($response->isAcceptedPayment()) {
                ...
                if ($save_on_failure || Scellius::isOney($response)) {
                    ...
                    $this->scelliusRedirect('index.php?controller=history');
                } else {
                    ...
                    if (! $response->isCancelledPayment()) {
                        ...
                        $this->context->cookie->scelliusPayErrors = $this->module->l('Your payment was not accepted. Please, try to re-order.', 'submit');
                    }
            ...
            $new_state = (int) Scellius::nextOrderState($response, false, $outofstock);
            ...
            if (! Configuration::get('PS_OS_OUTOFSTOCK_PAID')) {
                $bc_os_state_valid &= Scellius::isStateInArray($new_state, array('PS_OS_OUTOFSTOCK', 'SCELLIUS_OS_PAYMENT_OUTOFSTOCK'));
            }
            ...
            } elseif (Scellius::isStateInArray($old_state, Scellius::getManagedStates())) {
                if (($old_state == Configuration::get('PS_OS_ERROR')) && $response->isAcceptedPayment() && Scellius::hasAmountError($order)) {
    ...
    private function redirectSuccess($order, $check = false)
    {
        ...
        if (Configuration::get('SCELLIUS_MODE') === 'TEST') {
            if ($check) {
                // TEST mode (user is the webmaster): IPN did not work, so we display a warning.
                $link .= '&check_url_warn=yes';
            }

            if (ScelliusTools::$plugin_features['prodfaq']) {
                $link .= '&prod_info=yes';
            }
        }

        $this->scelliusRedirect($link);
    }
